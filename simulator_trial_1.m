...
f = @(t) [0.00*exp(-5*t); 0.000*exp(-5*t); 0.000*exp(-5*t); 0.00*exp(-5*t)];
...
dH_dtheta = @(theta) [
    -sqrt(a^2+b^2)*sin(atan(a/b) + theta(1)/2),  -R2;
    -R1,                                         0;
    R1,                                          0;
    R1,                                          R2
];
...
N_theta = @(th) [-(m1*g*lc1*cos(th(1)) + m2*g*(L1*cos(th(1))+lc2*cos(th(1)+th(2))));
